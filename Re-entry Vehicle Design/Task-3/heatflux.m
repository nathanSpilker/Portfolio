...
L = (625e-3)/cosd(theta); % [m] length of wedge side
...
%% More info about trajectory
q_a = 0.5*rho_1.*((V_1).^2); % dynamic pressure [N/m^2]
a_a = diff(v)./diff(t); % deceleration [m/s^2]
...
% max deceleration plot with label
max_decel = min(a_a);
alt_max_decel = z(find(a_a == min(a_a)))/1000;

figure()
hold on
plot(a_a,z(2:end,:)/1000)
xlabel('Deceleration (m/s^2)')
ylabel('Altitude (km)')
title('Deceleration vs. Altitude');
plot(max_decel,alt_max_decel,'.','MarkerSize',10);
text([max_decel+2],[alt_max_decel+2],{"("+max_decel+", "+alt_max_decel+")"},...
...
% radiation constants
epsilon = 0.88; % approximate emissivity for ceramic []
...
xlabel('T_w (K)')
ylabel('Altitude (m)')
title('Wall Temperature [K] at Stagnation based on Radiation Calcs');
...
C_coeff = 4.03e-5; % might also be 4.03e-9 or 2.53e-9 or 2.85e-5
pos = s-R_n*cosd(phi_range);

T_w_wedge = zeros(length(z),length(pos));
h_w_wedge = zeros(length(z),length(pos));
C_wedge = zeros(length(z),length(pos));
q_w_wedge_TM = zeros(length(z),length(pos));
T_w_wedge(1,:) = T_1(1)*ones(1,length(pos));
for i = 1:length(z)
    h_w_wedge(i,:) = c_p*T_w_wedge(i,:);
    C_wedge(i,:) = C_coeff*(cosd(theta)^0.5)*sind(theta)*...
    (s.^(-0.5))'.*(1-T_w_wedge(i,:)/T_aw_wedge(i));
    q_w_wedge_TM(i,:) = tm(C_wedge(i,:),rho_1(i),V_1(i),N,M_nose);
    if i < length(z)
...
xlabel('T_w,wedge (K)')
ylabel('Altitude (m)')
title('Wall Temperature [K] for Cone Section based on Radiation Calcs');
