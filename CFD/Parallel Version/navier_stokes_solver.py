...
def plot_results(U, V, P, mesh, x_m, y_m, center, L_x, L_y, n_x, n_y, init_vals, cond_type):
    ...
    # Apply the BCs
    U_val, V_val = apply_BCs(U_val, V_val, init_vals, cond_type)
...
def main():
    ...
    for itr in range(0, iters):
        if rank == 0:
            ...
            U_star, V_star = calc_Ui(U, V, nu, dt, dx, dy, init_vals, cond_type)
        else: 
            U_star = np.empty(U.shape)
            V_star = np.empty(V.shape)
        # Solve pressure poisson
        P = solve_poisson(input_solver, U_star, V_star, rho, dt, dx, dy, init_vals, cond_type, EPS, MAXITR, comm, size, rank)
        # Apply correction step to velocities
        if rank == 0:
            U, V = calc_Uf(U_star, V_star, rho, P, dt, dx, init_vals, cond_type)
