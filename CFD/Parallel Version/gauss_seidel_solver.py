...
def gauss_seidel_parallel(f, dx, EPS, MAXITR, comm, rank, size):
    ...
    while itr < MAXITR and gerr > EPS:
        ...
        # Even checkboards
        U_checker = np.copy(U_local)
        U_checker[1::2, 1::2] = 0
        U_checker[0::2, 0::2] = 0
        f_checker = np.copy(f_local)
        f_checker[1::2, 0::2] = 0
        f_checker[0::2, 1::2] = 0

        U_next[1:-1, 1:-1] = (1/4)*(U_checker[2:,1:-1] + U_checker[0:-2,1:-1] + U_checker[1:-1,2:] + U_checker[1:-1,:-2] - dx**2*(f_checker[1:-1,1:-1]))
        U_next+= U_checker
...
def gauss_seidel(f, dx, EPS, MAXITR, comm, rank, size):           
    P_sol = gauss_seidel_parallel(f, dx, EPS, MAXITR, comm, rank, size)
    return P_sol
