...
json_template = {"user1": "", "user2": "", "user3": "", "user4": "", "lcode1": "", "lcode2": "", "lcode3": "", "lcode4": "", "hand1": [], "hand2": [], "hand3": [], "hand4": [], 
                ...
                "OutOfCards1": 0, "OutOfCards2": 0, "OutOfCards3": 0, "OutOfCards4": 0, "ScoreThisRound1": 0, "ScoreThisRound2": 0, "passed": False, "aiPlayer1": False, "aiPlayer2": False, "aiPlayer3": False, "aiPlayer4": False}
    ...
    while True:
        ...
        if textJson != "":
            ...
            if textJson['event'] == 'spotDecision':
                ...
                addSpot(gameCode, userName, logoCode, spot)
        ...
        if textJson != "":
            ...
            if textJson['event'] == 'RegisterForChat':
                ...
                if ws not in client_list_chat[str(gameCode)]:
                    client_list_chat[str(gameCode)][str(userName)] = ws
            ...
            def getWS(gameCode, ind):
                return client_list_chat[str(gameCode)][data['user' + str(ind)]]


def addSpot(gameCode, username, logoCode, spot):
    jsonFile = open(str(gameCode) + ".json", 'r+', encoding='utf-8')
    data = json.load(jsonFile)
    ...
    try:
        while True:
            ...
            if textJson != "":
                ...
                if textJson['event'] == "RegisterForGame":                          
                    if username not in client_list_game[str(gameCode)]:

                        client_list_game[str(gameCode)][str(username)] = ws
                        client_connected_to_game[str(gameCode)][str(username)] = True
                        dataSend = {}
            ...
            def getWS(gameCode, ind):
                return client_list_game[str(gameCode)][data['user' + str(ind)]]
