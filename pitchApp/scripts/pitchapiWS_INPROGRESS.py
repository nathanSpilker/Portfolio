        ...
        if textJson != "":
            ...
            if textJson['event'] == 'registerForGame':
                ...
                if ws not in client_list[str(gameCode)]:
                    client_list[str(gameCode)][str(uid)] = ws
            ...
            if textJson['event'] == 'beginGame':
                ...
                wsDict = client_list[str(gameCode)]
                for uid in wsDict:
                    wsDict[uid].send(dataSend)

                del client_list[str(gameCode)]
                
            if textJson['event'] == 'updateSocketConnection':
                uid = textJson['uid']
                
                uidAttachedToGame = False
                for gameCodeKey in client_list.keys():
                    if uid in client_list[gameCodeKey].keys():
                        client_list[gameCodeKey][str(uid)] = ws
                        uidAttachedToGame = True
                        break
                    
                if not uidAttachedToGame:
                    dataSend = {}
                    dataSend['event'] = 'noGameForUid'
    ...
    while True:
        ...
        if textJson != "":
            ...
            if textJson['event'] == 'RegisterForChat':
                ...
                ws1 = getChatWS(gameCode, 1)
                ws2 = getChatWS(gameCode, 2)
                ws3 = getChatWS(gameCode, 3)
                ws4 = getChatWS(gameCode, 4)
    ...
    try:
        while True:
            ...
            if textJson != "":
                ...
                goRegApi = False
                if str(gameCode) not in client_list_game.keys():
                    goRegApi = True
                elif client_connected_to_game[str(gameCode)] == {}:
                    goRegApi = True
                else:
                    uidKeys = list(client_connected_to_game[str(gameCode)].keys())
                    if len(uidKeys) < 4:
                        goRegApi = True
                    elif (client_connected_to_game[str(gameCode)][uidKeys[0]] and client_connected_to_game[str(gameCode)][uidKeys[1]] and client_connected_to_game[str(gameCode)][uidKeys[2]] and client_connected_to_game[str(gameCode)][uidKeys[3]]):
                        goRegApi = True
                    elif textJson['event'] == "RegisterForGame":
                        goRegApi = True

                if goRegApi:
                    if textJson['event'] == "RegisterForGame":
                        if str(gameCode) in client_list_game.keys():                          
                            if uid not in client_list_game[str(gameCode)]:
                                ...
                                uidArr = list(client_connected_to_game[str(gameCode)].keys())
                                if (len(uidArr) == 4 and (not client_connected_to_game[str(gameCode)][uidArr[0]] or not client_connected_to_game[str(gameCode)][uidArr[1]] or not client_connected_to_game[str(gameCode)][uidArr[2]] or not client_connected_to_game[str(gameCode)][uidArr[3]])):
                                    prevDisconnected = True

                                client_connected_to_game[str(gameCode)][str(uid)] = True

                                nowConnected = False
                                if (len(uidArr) == 4 and (client_connected_to_game[str(gameCode)][uidArr[0]] and client_connected_to_game[str(gameCode)][uidArr[1]] and client_connected_to_game[str(gameCode)][uidArr[2]] and client_connected_to_game[str(gameCode)][uidArr[3]])):
                                    nowConnected = True

                                if nowConnected and prevDisconnected:
                                    dataSend = {}
                                    dataSend["event"] = 'UserReconnect'
                                    ws1 = client_list_game[str(gameCode)][data['uid' + str(1)]]
                                    ws2 = client_list_game[str(gameCode)][data['uid' + str(2)]]
                                    ws3 = client_list_game[str(gameCode)][data['uid' + str(3)]]
                                    ws4 = client_list_game[str(gameCode)][data['uid' + str(4)]]
                                    ws1.send(dataSend)
                                    ws2.send(dataSend)
                                    ws3.send(dataSend)
                                    ws4.send(dataSend)
                        ...
                        ws1 = getGameWS(gameCode, 1)
                        ws2 = getGameWS(gameCode, 2)
                        ws3 = getGameWS(gameCode, 3)
                        ws4 = getGameWS(gameCode, 4)
                    ...
                    dataSend = {}
                    dataSend["event"] = "UserDisconnect"

                    for userInd in range(1,5):
                        if (not client_connected_to_game[gameCode][data['uid' + str(userInd)]]):
                            dataSend["user"] = data['uid' + str(userInd)]

                    for userInd in range(1,5):
                        if (client_connected_to_game[gameCode][data['uid' + str(userInd)]]):
                            wsTemp = getGameWS(gameCodeInfo, userInd)
                            wsTemp.send(dataSend)
            ...
            def getGameWS(gameCode, ind):
                return client_list_game[str(gameCode)][data['uid' + str(ind)]]

    except ConnectionClosed:
        print(userInfo + " disconnected.")   
        # TODO: handle reconnecting gracefully
        client_connected_to_game[gameCodeInfo][uidInfo] = False
        
        dataSend = {}
        dataSend["event"] = "UserDisconnect"
        dataSend["user"] = userInfo 

        for userInd in range(1,5):
            if (client_connected_to_game[gameCodeInfo][data['uid' + str(userInd)]]):
                wsTemp = client_list_game[str(gameCodeInfo)][data['uid' + str(userInd)]]
                wsTemp.send(dataSend)
        ...
        if not userStillConnected:
            ...
            del client_list_game[gameCodeInfo]
            del client_list_chat[gameCodeInfo]
