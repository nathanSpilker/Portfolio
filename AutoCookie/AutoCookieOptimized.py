...
def close_to_edge(row, col, dist):
    maxRow = min(row + dist, height - 1)
    minRow = max(row - dist, 0)
    maxCol = min(col + dist, width - 1)
    minCol = max(col - dist, 0)

    for coord in zeros:
        if coord[0] <= maxRow and coord[0] >= minRow and coord[1] <= maxCol and coord[1] >= minCol:
...
candidate = candidate.resize((basewidth,hsize), Image.LANCZOS)
...
padded_candidate = padded_candidate.resize((basewidth,hsize), Image.LANCZOS)
